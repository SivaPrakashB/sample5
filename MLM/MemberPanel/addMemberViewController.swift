...
class addMemberViewController: UIViewController,UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,XMLParserDelegate{
 ...
 */
    ...
    @objc func keyboardDidShow(notification: Notification) {
        ...
        print(keyboardY,"keyboardY")
        let editingTextFieldY:CGFloat! = self.activeTextField.frame.origin.y
                print(editingTextFieldY,"editingTextFieldY")
        if self.view.frame.origin.y >= 0 {
                            print(self.scrollView1.frame.origin.y,"self.scrollView1.frame.origin.y")
            //Checking if the textfield is really hidden behind the keyboard
            if editingTextFieldY > keyboardY - 60  {
                UIView.animate(withDuration: 0.25, delay: 0.0, options: UIViewAnimationOptions.curveEaseIn, animations: {
                 self.view.frame = CGRect(x: 0, y: 0  - (keyboardY - 60), width: self.view.bounds.width,height: self.view.bounds.height)
                }, completion: nil)
            }
        }
